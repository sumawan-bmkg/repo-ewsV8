...
def plot_probability_kde(results: list, y_true: np.ndarray,
    ...
    for ax, result in zip(axes, results):
        ...
        ax.hist(result.probs[mask_pos], bins=80, density=True, alpha=0.6,
                color='#2ca02c', label=f'Gempa (n={mask_pos.sum()})',
                edgecolor='white', linewidth=0.5)
        ax.hist(result.probs[mask_neg], bins=80, density=True, alpha=0.6,
                color='#d62728', label=f'Noise (n={mask_neg.sum()})',
                edgecolor='white', linewidth=0.5)
    ...
    for result, color in zip(results, colors):
        if result.probs.ndim == 1 and len(result.probs) > 0:
            prob_true, prob_pred = calibration_curve(
                y_true, result.probs, n_bins=15, strategy='uniform'
            )
...
def main():
    ...
    TEMPERATURE = 0.05   # T < 1 untuk memperkuat kontras
